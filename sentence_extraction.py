...
class SentenceData(object):
    def __init__(self, sentence_text, docID, label):
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer):
    ...
    for example in tqdm(examples,desc='Convert To Features'):
        ...
        for (sent_index,sent_example) in enumerate(example.sentence_data):
            tokens_sentence = tokenizer.tokenize(sent_example.sentence_text)
        ...
        # the entire model is fine-tuned.
            ...
            label_id = label_map[str(sent_example.label)]
            if logging_count < 5:
                logging_count += 1
                logger.info("*** Example ***")
                ...
                logger.info("label: %s (id = %d)" % (sent_example.label, label_id))
